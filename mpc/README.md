...
| | 成功 | 碰撞 |
|---|---|---|
| 变种2 `mpc_grid` | **1.000** | 0.000 |
| 变种1 `mpc_vw`（内置**紧**场景） | 0.592 | 0.408 |
...
| `LiveConfig.magnitude` | 20 | 实车 PWM 幅值。**摩擦阈值 17 PWM**，所以 mag 20 只有 v_max≈0.04 m/s（几乎不动）；**实用值 40**（≈0.31 m/s），GUI 默认也是 40 |
| `LiveConfig.collision_abort` | True | 逼近碰撞软停（GUI/smoke 默认**关**，障碍圆自带 margin）|
| `RobotConfig.pwm_per_mps / pwm_offset` | 73.4 / 17.0 | **实测**仿射被控对象 `轮速=(PWM−17)/73.4`。见上面的标定小节 |
| `RobotConfig.wz_arm`, `Variant1Config.steer_arm` | 0.194 | **实测**偏航臂（米），使指令 w == 实际偏航率 |
| `Variant1Config.w_max` | 1.2 | 偏航上限 → 最小转弯半径 = v/w：mag 40 → **0.26 m** |
| `MPPIConfig.horizon / dt` | 29 / 1/3 | `dt` **就是一个 tick**（执行的正是第一步）；H·dt ≈ 9.7 s 前视，mag 40 下 ≈3.0 m |
...
## 标定被控对象模型（`smoke/calib_model.py`）

planner 的模型必须和真车一致,否则闭环性能靠运气。**电机有静摩擦,所以关系是仿射的,不是比例的**:
...
```bash
roscar
python smoke/calib_model.py --test yaw                # 原地旋转,不需要空间
python smoke/calib_model.py --test linear             # 需要前方 ~1.5 m
python smoke/calib_model.py --test arc                # 端到端校验(含死区补偿)
python smoke/calib_model.py --all --mags 30,40,60 --secs 1.5
```
参考基准是 `/odom`(xy 来自编码器、已用激光标定;yaw 来自 IMU 陀螺)。Ctrl-C 或任何异常都会硬急停。
...
| `arc` | `v>0` 且带偏航,过 `velocity_to_wheel_pwm` | 端到端实际值 vs 指令 |

**必须至少测 3 个 PWM 档**,否则分不清"斜率错"和"有偏置"。对 `v` 和 `w` 各做一次线性拟合,截距就是 `pwm_offset`,斜率的倒数就是 `pwm_per_mps`;偏航臂 = `((P−offset)/k) / w_实测`。

### 2026-07-25 的实测结果

```
LINEAR  v = (P − 17.0) / 73.4          拟合残差 ±0.003 m/s (PWM 30/40/60)
YAW     偏航轴独立解出的偏置 = 18.4 PWM   ← 和直线轴的 17.0 吻合
        仿射模型下 arm = 0.199 / 0.195 / 0.186 → 0.194 m
        旧的比例模型 arm = 0.168 / 0.125 / 0.095 ← 根本不是常数
```
> **一个摩擦偏置会伪装成错误的几何参数。** 如果你算出来的 arm 随 PWM 变化,那不是几何问题,是你漏了偏置。

PWM 40(实际驱动档位)的旧模型误差:速度 **1.55×** 偏快,偏航 **0.80×** 偏慢 —— 合起来实际转弯半径约为规划值的 1.9 倍,这就是变种1 转圈跑飞的原因。
...
```
斜率只差 1.8%、arm 差 2%,**摩擦阈值差最多**(电压高时更低的 PWM 就能克服静摩擦)。
arm 是同一电压下两轴的比值,电压被消掉了,所以两次高度一致 —— **这是最该信的数**。

### 标定之后仍未建模的两项(实测)

| 现象 | 数据 | 影响 |
|---|---|---|
| **v–w 耦合** | `arc` 测试 v=0.361:直行 99%,w=0.6 时 90%,w=1.2 时 **53%** | 滑移转向蹭掉前进速度,而 `rollout_unicycle` 假设 v 与 w 无关。初步可用 `v_eff = v·(1−0.30·w²)` |
| **偏航滞后** | 成功那次跑的累积 measured/cmd:平移 0.810,**偏航 0.451** | 指令在 tick 间来回翻(+0.68/+0.50/−0.40/−0.53),车的惯性跟不上。就是笔记里那句"ω jitter",现在有数了 |

### ⚠️ 两个坑

1. **yaw 必须运行中连续积分,不能用首尾差。** `/odom` 的 yaw 缠绕在 (−π, π],转过半圈读数就废了 —— 第一次跑 PWM 60 转了整整一圈多,首尾差读出 −3.4°,把 arm 算成了 −7.165 m。脚本已改为每个脉冲采样 6 次累加。
2. **电池电压影响 `pwm_per_mps`。** 上面是 **9.8 V** 下测的(健康是 11–13 V),给定 PWM 的转速随电压下降。**充满电后要重跑 `--test linear`**。`pwm_offset`(阈值)和 `wz_arm`(比值)对电压不敏感,不用重测。

### 死区补偿:加常数,不要逐轮钳位

旧实现把每个轮子**独立**抬到 `deadzone_pwm=30`,这会**抹平转向差分**(差分就是全部的转向信号):mag 20 时 |w|≤0.45 实际输出 0 rad/s。

正确做法是仿射逆:`PWM = v·k + offset·sign(v)` —— 给每个轮子加**同一个**常数,轮间差值不变,`(P−offset)/k` 反解回去正好是指令值。实测指令→实际偏航率:

| magnitude | 修复前 | 修复后 |
|---|---|---|
| 20 | 0% / 0% / 13% / 17% | **100%** |
| 40 | 100% / 100% / 87% / 78% | **100%** |

### 标定之后的离线结果(20 seeds × 场景)

| | success | collide |
|---|---|---|
| 变种2(内置紧场景) | 0.833 → **1.000** | 0.000 |
| 变种1(内置紧场景) | 0.750 → **0.592** | 0.250 → 0.408 |
| 变种1(**真实场景** B=3m,障碍 1.2–1.5m) | **1.000** | **0.000** |

变种2 变好是因为跳格距离原本被高估了 ~55%。变种1 在**紧场景**变差不是退化 —— 把 `steer_arm` 改回虚构的 0.10 就能复现旧的 0.750,**说明旧数字是假的**:仿真在给它做真车做不到的急转。内置紧场景(障碍 0.5 m、B=1 m)要求的转弯半径本来就超出车的物理能力;在实际驱动的场景里,标定后的模型是 1.000 / 0 碰撞,和旧模型完全一样。
