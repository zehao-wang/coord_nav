...
- **四个内置 policy**（都连续操控、到点前不停）：
  | key | 名字 | 控制 | 走哪个话题 |
  |---|---|---|---|
  | `mpc_grid` | 变种2（离散 baseline，**冻结世界**） | 离散"跳格子"：12 跳视野**采样 MPC**（游程结构候选 + 暖启动 + 2 轮精化） | `/drive_action` |
  ...
  | `orca` | **外部基线**：ORCA/RVO2（文献标准反应式 crowd 策略） | 吃追踪器的 (位置,速度,半径),输出全向速度→就近离散动作 | `/drive_action` |

> `orca` 需要 Python-RVO2 绑定（源码编译,见 CHANGELOG 0.9.23）；纯反应式、假设互惠让行,
> 静态口袋/窄缝是它的已知弱项 —— 榜单会如实呈现这种"动态强静态弱"画像。
